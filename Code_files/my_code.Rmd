...
p3_calendar <- 
  ...
  geom_tile( # Draw rectangular tiles for each date (in LOW range)
    ...
    fill = 'purple', # colors outliers purple
    color = "white", # makes white borders between tiles
    width = 6.5, # sets tile width
    height = 0.95, # sets tile height
    linewidth = 0.6) + # sets thickness of tile borders
  ...
  scale_fill_gradient( # Define color gradient for sleep duration
    ...
    limits = c(sleep_range$min_sleep, sleep_range$max_sleep)) + # limits range
  ...
  labs(
    ...
    caption = "Purple tile = outlier low-sleep day (excluded from color-scale)",
...
p4_bubble <-
  ...
  geom_point( # creates outlier point excluded from color-scale
    ...
    color = "blue", # colors outlier blue
    size = 3) + # sets size to 3
  scale_x_continuous(labels = comma) + # formats x-axis with commas
  scale_color_gradient( # scales color by time stressed
    low = "#FEE5D9", # sets lowest stress color
    high = "#A50F15", # sets highest stress color
    name = "Time Stressed (min)", # names legend
    limits = c(stress_limits$min_stress, # sets lower limit for color scale
               stress_limits$max_stress)) + # sets upper limit for color scale
