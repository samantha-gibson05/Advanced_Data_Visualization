...
p2_sleep_scatter <- 
  ...
  scale_fill_viridis_c(name = "Number of days", # creates legend title 
                       option = "C") + # colors by cohesive color-scheme
  labs(
...
p3_calendar <- 
  ...
  geom_tile( # Draw rectangular tiles for each date (in LOW range)
    ...
    fill = 'cyan', # colors outliers green
    color = "white", # makes white borders between tiles
    width = 6.5, # sets tile width
    height = 0.95, # sets tile height
    linewidth = 0.6) + # sets thickness of tile borders
  scale_x_date(date_labels = "%b %d") + # display axis format like "Jan 09"
  scale_fill_viridis_c( # Define color gradient for sleep duration
...
p4_bubble <-
  ...
  geom_point( # creates outlier point excluded from color-scale
    ...
    color = "cyan", # colors outlier blue
    size = 3) + # sets size to 3
  scale_x_continuous(labels = comma) + # formats x-axis with commas
  scale_color_viridis_c(
    name = "Time stressed (min)",
    option = "C",
    limits = c(stress_limits$min_stress, stress_limits$max_stress)) +
  labs(title = "Steps vs Sleep (Colored by Time Stressed)", # sets title
    caption = "Blue point = low-sleep outlier", # creates caption
...
```
 xxxx custom different palletxxx
